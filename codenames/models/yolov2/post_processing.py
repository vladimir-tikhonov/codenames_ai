...
def generate_yolo_grid(g: np.ndarray, num_bb: np.ndarray) -> np.ndarray:  # pylint: disable=invalid-name
...
class PostProcessing:
    ...
    def __call__(self, y_sing_pred: np.ndarray) -> np.ndarray:
        num_bounding_boxes = len(self.config['anchors']) // 2
        c_grid = generate_yolo_grid(self.config['gridSize'], num_bounding_boxes)
        anchors = np.reshape(self.config['anchors'], [1, 1, 1, num_bounding_boxes, 2])

        # need to convert b's from gridSize units into IMG coords. Divide by grid here.
        b_xy = (keras.sigmoid(y_sing_pred[..., 0:2]) + c_grid[0]) / self.config['gridSize']
        b_wh = (keras.exp(y_sing_pred[..., 2:4]) * anchors[0]) / self.config['gridSize']
        b_xy1 = b_xy - b_wh / 2.
        b_xy2 = b_xy + b_wh / 2.
        ...
        indicator_detection = scores_all > self.config['detectionThreshold']
        ...
        s2b = (self.config['gridSize'] ** 2) * num_bounding_boxes
        ...
        nms_ids = tf.image.non_max_suppression(
            flatten_boxes,
            flatten_scores * mask,
            max_output_size=self.config['maxBoxes'],
            iou_threshold=self.config['iouThreshold'],
            score_threshold=self.config['scoreThreshold']
        )
